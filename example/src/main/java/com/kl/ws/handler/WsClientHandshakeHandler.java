...
public class WsClientHandshakeHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        log.info("[WsClientHandshakeHandler][channelActive] 通道连接");
        // 构建和发送握手请求
        handshaker.handshake(ctx.channel());
        log.info("[WsClientHandshakeHandler][channelActive] 通道连接成功 channel={}", ctx.channel());
    }
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        log.debug("[WsClientHandshakeHandler][channelRead] 接收到数据");
        // 判断握手是否完成
        if (!handshaker.isHandshakeComplete()) {
            try {
                // 如果握手未完成，则完成握手(验证响应、升级通道、设置握手完成标志)
                handshaker.finishHandshake(ctx.channel(), (FullHttpResponse) msg);
                log.info("[WsClientHandshakeHandler][channelRead] 握手完成");
                // 设置握手结果
                handshakeResult.setSuccess();
            } catch (Exception e) {
                log.error("[WsClientHandshakeHandler][channelRead] 握手异常", e);
                // 设置握手结果
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        log.error("[WsClientHandshakeHandler][exceptionCaught] 异常处理", cause);
